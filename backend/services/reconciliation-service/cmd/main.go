...
import (
	...
	"stock_trader/backend/services/reconciliation-service/internal/interfaces/http"
...
func (a *Application) Initialize() error {
	a.logger = logger.NewLogger("reconciliation-service")
	a.logger.Info("初始化对账服务...")
	...
	discrepancyRepo := repository.NewDiscrepancyRepository(nil, a.logger)
	reportRepo := repository.NewReportRepository(nil, a.logger)
	fixRepo := repository.NewFixRecordRepository(nil, a.logger)

	a.domainService = service.NewReconciliationDomainService(
		a.cfg,
		a.logger,
		discrepancyRepo,
		reportRepo,
	...
	a.logger.Info("对账服务初始化完成")
	return nil
}

func (a *Application) createTables() error {
	a.logger.Info("数据库表已就绪")
	return nil
}
